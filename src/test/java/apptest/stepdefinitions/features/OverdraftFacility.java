package apptest.stepdefinitions.features;

import apptest.pages.Common_Page;
import io.cucumber.java.en.Given;

public class OverdraftFacility {
	@Given("scenario Overdraft Facility ends")
	public void scenario_overdraft_facility_ends() throws Exception {
		try {

		} catch (Exception e) {
			Common_Page.setScenarioResult(false);
		} finally {
			if (!Common_Page.isScenarioResult()) {
				throw new Exception("Scenario 'Overdraft Facility' FAILED");

			}
		}

	}

}